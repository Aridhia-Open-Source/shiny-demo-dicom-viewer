...
shinyServer(function(input, output, session) {
  ...
  shinyDirChoose(input, "dicom_dir", 
                 roots = c(scripts = file.path(home, "scripts"),
                           documents = file.path(home, "documents"),
                           datafiles = file.path(home, "datafiles")),
                 filetypes = c(""))
  
  session_values <- reactiveValues(selected_file = NULL)
  
  observeEvent(input$dicom_dir, {
    if(is.numeric(input$dicom_dir)) {
      return()
    } else {
      filename <- do.call(file.path, c(home, input$dicom_dir$root, input$dicom_dir$path[-1]))
      session_values$selected_file <- filename
    }
  })
  ...
  dcm_images <- reactive({
    dicoms <- session_values$selected_file
    if(is.null(dicoms)) {
      return(NULL)
    }
